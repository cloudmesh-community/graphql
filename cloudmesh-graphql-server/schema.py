...
from models import AWSImage as AWSImageModel
from models import AzureImage as AzureImageModel
...
class AzureImage(MongoengineObjectType):
    class Meta:
        model = AzureImageModel
        interfaces = (Node,)

class AWSImage(MongoengineObjectType):
    class Meta:
        model = AWSImageModel
        interfaces = (Node,)
...
schema = graphene.Schema(query=Query, mutation=Mutation, types=[AWS, Azure, AzureImage, AWSImage])